...
def write_entry_to_file(entry):
    with open('database.csv', mode='a+', newline='') as file:
        ...
        file_writer.writerow([entry['timestamp'], entry['vehicle_id'],
                                entry['changed_state'], entry['new_state']])
...
def create_entry():
    ...
    ts = str(dt.year) + '-' + str(dt.month) + '-' + str(dt.day) + '_' + str(
    ...
    # """vehicle_id""": """12345"""}" http://localhost:5000/api
    # curl -i -H "Content-Type: application/json" -X POST -d "{
    # """vehicle_id""": """12345"""}" https://blg-challenge.herokuapp.com/api
...
def delete_database():
    ...
    # curl -i -H "Content-Type: application/json" -X DELETE -d "
    # " https://blg-challenge.herokuapp.com/api
